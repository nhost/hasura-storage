...
func getGin(
	...
	av, err := getAv(viper.GetString(useClamavFlag))
	if err != nil {
...
func init() {
...

	{
		...
		addStringFlag(
			serveCmd.Flags(), useClamavFlag, "", "Use ClamAV to scan files. Example: tcp://clamavd:3310",
		)
	}
}
	...
	Run: func(_ *cobra.Command, _ []string) {
		...
		logger.WithFields(
			logrus.Fields{
				...
				useClamavFlag:          viper.GetString(useClamavFlag),
			},
		).Debug("parameters")

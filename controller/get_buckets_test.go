...
func TestGetBuckets(t *testing.T) {
	...
	cases := []struct {
		name             string
		expectedStatus   int
		expectedResponse *controller.GetBucketsResponse
	}{
		{
			name:           "good",
			expectedStatus: 200,
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			metadataStorage.EXPECT().GetBuckets(
			...
			assertJSON(t, responseRecorder.Body.Bytes(), &controller.GetBucketsResponse{}, tc.expectedResponse)
		})
	}
}
